...
class RobotAgent(UEAgent):
    ...
    def combine_processed_observations(self, observations: Dict[str, Any]) -> Any:
        ...
        return observations["cpu_camera"]
...
def main():
    ...
    # ----------------------------------------------------------------------------
    agent_obs_space = gymnasium.spaces.Box(
        low=0.0,
        high=255.0,
        shape=(42, 42, 3),
        dtype=np.uint8
    ...
    config = (
        ...
        .training(                  # This version of Ray is type hinting a generic training config, 
            ...
            lr=3e-4,                                # type: ignore
            train_batch_size=terminator_conf.max_step_count * 5, # Training iteration every 5 worker episodes
